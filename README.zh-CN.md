...
- **直接指定凭证**：保存一个或多个精确、区分大小写的 Scheduler Auth ID；无需创建分类规则。选择器同时列出 Auth 目录凭据，以及 CPA「AI 提供商」中的 Gemini、Interactions、Claude、Codex、xAI、Vertex 和 OpenAI-compatible 配置凭据。适合凭证较少、每个 Key 单独分配的场景。多选是允许列表，不是轮询顺序；新增凭证不会自动加入。
