...
- CPA activates only the highest-priority Scheduler plugin. `access-guard` must be the winning Scheduler for isolation to run.
...
- **Direct credentials** persist one or more exact, case-sensitive Scheduler Auth IDs without creating a classify rule. The picker includes both auth-directory credentials and CPA AI-provider entries for Gemini, Interactions, Claude, Codex, xAI, Vertex, and OpenAI-compatible providers. This is best for a small per-key allow-list. Selection defines the allowed pool, not its rotation order, and new credentials are not added automatically.

Native bindings have an optional **Concurrent round robin** switch (`round_robin`, default `false`). Enable it on a dedicated translation key and keep its model allow-list restricted to the desired model. Requests rotate among that binding's eligible credentials, with a separate cursor for each caller scope, provider pool, and model. Concurrent requests can execute on different credentials; one request still selects only one credential at a time. Set the credentials to the same priority so CPA can offer all of them to the plugin.
...
`plugins/linux/amd64/access-guard.so`, and enable the plugin in config.
The library basename must be `access-guard` (optionally
`access-guard-v<version>`); putting `_linux_amd64` in the basename changes
the plugin ID and prevents `plugins.configs.access-guard` from matching it.
...
2. Enable `plugins` + `access-guard` in CPA config; set `state_file`.
